...
public class WaterBudget {
	...
	//	double out = Math.max(c,recharge) * Math.pow(Sn  / s_RunoffMax, d);
		double out = c * Math.pow(Sn  / s_RunoffMax, d);
	...
	public double[] RK4(double Sn) {
		...
		for (int k = 0; k < RKiter; k++) {
			...
			double deltaRunoff = dt * computeRunoff(Sn);
			balance = balance + Sn - Sn1 + dt * recharge - deltaRunoff;
			runoff = runoff + deltaRunoff;
			
			Sn = Sn1;
		
		}
		return new double[] { Sn, balance, runoff };
	}
